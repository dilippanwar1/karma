...
var ProcessLauncher = function (spawn, tempDir, timer) {
  ...
  this._execCommand = function (cmd, args) {
    ...
    self._process.on('exit', function (code) {
      self._onProcessExit(code, errorOutput, stdout, stderr)
    })

    self._process.stdout.on('data', function (chunk) {
      stdout += chunk.toString()
    })

    self._process.stderr.on('data', function (chunk) {
  ...
  this._onProcessExit = function (code, errorOutput, stdout, stderr) {
